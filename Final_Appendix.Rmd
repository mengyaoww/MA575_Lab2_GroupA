...
```{r, fig.height=9}
key_model <- lm(price ~ mileage + engine_power + model_series, data = dat_bmw_clean)
summary(key_model)
...
Based on the curved line observed in the "Residual vs Fitted" Plot in the "key_model" and the quadratic relationship between price and mileage, we add $\sqrt{mileage}$ to the model: $price = \beta_0 + \beta_1 mileage + \beta_2 engine \hspace{1mm} power + \beta_3 model \hspace{1mm} series + \beta4 \sqrt{mileage} + \epsilon$. The fitted line in the "Residual vs Fitted" Plot is not as quadratic as it in the "key_model". The deviated tail may be caused by potential outliers.

```{r}
refine_model <- lm(price ~ mileage + engine_power + model_series + I(mileage^(1/2)), data = dat_bmw_clean)
...
We further added the interaction term between mileage and engine_power to see how engine_power influence the association between price and mileage. The model was $price = \beta_0 + \beta_1 mileage + \beta_2 engine \hspace{1mm} power + \beta_3 model \hspace{1mm} series + \beta4 \sqrt{mileage} + \beta_5 mileage \times engine \hspace{1mm} power + \epsilon$

```{r}
interact_model <- lm(price ~ mileage + engine_power + model_series + I(mileage^(1/2)) + mileage:engine_power, data = dat_bmw_clean)
...
final_model <- lm(formula = price ~ mileage + engine_power + model_series + I(mileage^(1/2)) + mileage:engine_power + age, data = dat_bmw_clean)
summary(final_model)
